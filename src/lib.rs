...
use model::map::{MapDetail, MapVersion};
...
lazy_static! {
    ...
    pub static ref BEATSAVER_URL: Url = Url::parse("https://api.beatsaver.com").unwrap();
}
...
impl Into<MapId> for &MapDetail {
    fn into(self) -> MapId {
        MapId::Key(self.id.clone())
    }
}

impl Into<MapId> for MapVersion {
    fn into(self) -> MapId {
...
mod tests {
    ...
    fn test_page() {
        ...
        let page: Page<MapDetail> = serde_json::from_str(data).unwrap();
